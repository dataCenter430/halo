...
/**
 * Builds a synthetic value for the hidden FormKit group so that conditionally
 * rendered groups (e.g. group with if: "$value.type === 'article'") are
 * actually mounted. Without this, node.at("article.postName") would be
 * undefined and item labels would fall back to raw values (e.g. IDs) instead
 * of resolved labels (e.g. post names).
 */
